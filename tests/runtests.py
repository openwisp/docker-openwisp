...
class TestBootstrap(BaseTestUtils, unittest.TestCase):
    def test_names_do_not_select_bootstrap_objects(self):
        """Ensure renamed bootstrap objects are resolved by their UUID selectors."""
        script = r"""
...
default_vpn = Vpn.objects.get(
    pk=client.get("openwisp_default_vpn_uuid").decode()
)
...
marker = "bootstrap-selector-decoy"
template_marker = "bootstrap-selector-template"
ssh_template_marker = "bootstrap-selector-ssh-template"
...
try:
    ...
    assert selected_vpn.pk == default_vpn.pk, (
    ...
    assert selected_topology.pk == default_topology.pk, (
    ...
    assert selected_template.pk == default_template.pk, (
    ...
    assert selected_ssh_template.pk == ssh_template.pk, (
    ...
    ssh_template.save()
    client.set("openwisp_default_ssh_template_uuid", str(ssh_template.pk))
    selected_ssh_template = load_init_data.create_ssh_key_template()
    assert selected_ssh_template.pk == ssh_template.pk, (
        "SSH template name created a duplicate"
    )
    print("bootstrap selectors passed")
finally:
...
"""
        output, _ = self._execute_django_shell_command(f"exec({script!r})")
        self.assertEqual(output.strip().splitlines()[-1], "bootstrap selectors passed")
